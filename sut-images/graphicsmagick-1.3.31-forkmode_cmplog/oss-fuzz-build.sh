...
make clean || echo "no clean"
autoreconf -fiv
PKG_CONFIG_PATH="$WORK/lib/pkgconfig" ./configure CPPFLAGS="-I$WORK/include" CFLAGS="$CXXFLAGS" LDFLAGS="${LDFLAGS:-} -L$WORK/lib" --enable-static --disable-shared  --prefix="$WORK"
make -j$(nproc)
make install
popd
