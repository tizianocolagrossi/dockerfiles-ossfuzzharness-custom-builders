...
static int dumpDataToPdfFile(const uint8_t* data, size_t size, const char* path)
{
	FILE* fd;
	int written = 0;
    ...
    error(errNotAllowed, -1,
      "Error opening Pdf file for writing");
		return -1;
	}
	...
	{
    error(errNotAllowed, -1,"Error writing Pdf file");
		fclose(fd);
		return -1;
	}
...
// int main(int argc, char *argv[]) {
  ...
  //   goto err0;
  // }
  dumpDataToPdfFile(data, size, "/tmp/dummy.pdf");
  fileName = new GString("/tmp/dummy.pdf");
  ...
  // }
  // // output to /dev/null hardcoded
  textFileName = new GString("/dev/null");
