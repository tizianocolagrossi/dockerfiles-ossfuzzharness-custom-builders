...
pushd /src/SGFuzz/
cat /src/SGFuzz/sanitizer/State_machine_instrument.py
git apply $SRC/check_escaped.patch
popd

python3 /src/SGFuzz/sanitizer/State_machine_instrument.py . -b $SRC/blocked_variables.txt

export CXXFLAGS="${CXXFLAGS//-lsFuzzer/}"
...
# Compile fuzzer.
$CXX $CXXFLAGS -I../include $LIB_FUZZING_ENGINE -lz -llzma -lsFuzzer \
...
$CXX $CXXFLAGS -DNDEBUG -I genfiles -I .. -I ../libprotobuf-mutator/ -I ../LPM/external.protobuf/include -I ../include $LIB_FUZZING_ENGINE \
    ...
    -lz -llzma -lsFuzzer -Wl,--start-group ../LPM/external.protobuf/lib/lib*.a -Wl,--end-group \
