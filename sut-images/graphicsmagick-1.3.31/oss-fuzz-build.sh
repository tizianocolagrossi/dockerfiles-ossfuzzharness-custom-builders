...
pushd "$SRC/xz"
./autogen.sh
PKG_CONFIG_PATH="$WORK/lib/pkgconfig" ./configure --disable-xz --disable-lzmadec --disable-lzmainfo --disable-lzma-links --disable-scripts --disable-doc --disable-shared --with-pic=yes --prefix="$WORK"
make -j$(nproc)
make install
popd
...
# ./autogen.sh
# PKG_CONFIG_PATH="$WORK/lib/pkgconfig" ./configure CPPFLAGS="-I$WORK/include" CFLAGS="$CFLAGS" LDFLAGS="${LDFLAGS:-} -L$WORK/lib" --enable-static --disable-shared --prefix="$WORK" CFLAGS="$CFLAGS"
# # ./configure --enable-static --disable-shared
# make -j$(nproc) CPPFLAGS="-DHB_NO_VISIBILITY" V=1 all
# make install
# export CFLAGS="$OLDCF"
# export CXXFLAGS="$OLDCXXF"
# popd
...
chmod +x ./autogen.sh
./autogen.sh
PKG_CONFIG_PATH="$WORK/lib/pkgconfig" ./configure CPPFLAGS="-I$WORK/include" CFLAGS="$CFLAGS" LDFLAGS="${LDFLAGS:-} -L$WORK/lib" --enable-static --prefix="$WORK" CFLAGS="$CFLAGS -fPIC"
make -j$(nproc)
make install
popd
...
PATH=$WORK:$WORK/bin:$PATH PKG_CONFIG_PATH="$WORK/lib/pkgconfig" ./configure CPPFLAGS="-I$WORK/include/libpng16 -I$WORK/include/freetype2 -I$WORK/include" CFLAGS="$CFLAGS" LDFLAGS="${LDFLAGS:-} -L$WORK/lib" --prefix="$WORK" --without-perl --with-quantum-depth=16
make "-j$(nproc)"
make install
...
# maximum extent possible. MANCA liblzma harfbuzz 
MAGICK_LIBS="-lharfbuzz -lXext -lX11 -lxml2 $WORK/lib/libpng.a $WORK/lib/libtiff.a $WORK/lib/liblcms2.a $WORK/lib/libwebpmux.a $WORK/lib/libwebp.a $WORK/lib/libturbojpeg.a $WORK/lib/libfreetype.a $WORK/lib/liblzma.a $WORK/lib/libz.a" #$SRC/lib/libharfbuzz.a"  $WORK/lib/libsharpyuv.a
...
for f in fuzzing/*_fuzzer.cc; do
    ...
    $CXX $CXXFLAGS -fsanitize=fuzzer -std=c++11 -I "$WORK/include/GraphicsMagick" \
...
for item in $("$WORK/coder_list"); do
    ...
    $CXX $CXXFLAGS -std=c++11 -I "$WORK/include/GraphicsMagick" \
        ...
        $coder_flags -fsanitize=fuzzer "$WORK/lib/libGraphicsMagick++.a" \
        "$WORK/lib/libGraphicsMagick.a" $MAGICK_LIBS
