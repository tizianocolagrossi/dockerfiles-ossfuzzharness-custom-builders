...
LINK_LIB_X="$(pkg-config --cflags --libs x11 fontconfig libxml-2.0 libpng cairo xext pango pangocairo) " 

$CXX $CXXFLAGS -std=c++11 $LINK_LIB_X -I"$WORK/include/ImageMagick-7" "$SRC/encoder_list.cc" \
    -o "$WORK/encoder_list" \
...
for f in $SRC/*_fuzzer.cc; do
    ...
    $CXX $CXXFLAGS -std=c++11 $LINK_LIB_X -I"$WORK/include/ImageMagick-7" \
